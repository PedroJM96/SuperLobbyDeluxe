...
public class PlayerListener implements Listener {
	...
	private final String[] buttons = { "ACACIA_BUTTON", "BIRCH_BUTTON", "DARK_OAK_BUTTON", "JUNGLE_BUTTON",
			"OAK_BUTTON", "SPRUCE_BUTTON", "STONE_BUTTON", "WOOD_BUTTON", "STONE_BUTTON", "CRIMSON_BUTTON",
			"WARPED_BUTTON", "MANGROVE_BUTTON", "BAMBOO_BUTTON", "CHERRY_BUTTON" };
	private final String[] trapdoors = { "ACACIA_TRAPDOOR", "BIRCH_TRAPDOOR", "DARK_OAK_TRAPDOOR", "IRON_TRAPDOOR",
			"JUNGLE_TRAPDOOR", "OAK_TRAPDOOR", "SPRUCE_TRAPDOOR", "TRAP_DOOR", "CRIMSON_TRAPDOOR", "WARPED_TRAPDOOR",
			"MANGROVE_TRAPDOOR", "BAMBOO_TRAPDOOR", "CHERRY_TRAPDOOR" };
	private final String[] doors = { "ACACIA_DOOR", "BIRCH_DOOR", "DARK_OAK_DOOR", "IRON_DOOR", "JUNGLE_DOOR",
			"OAK_DOOR", "SPRUCE_DOOR", "WOOD_DOOR", "WOODEN_DOOR", "CRIMSON_DOOR", "WARPED_DOOR", "MANGROVE_DOOR",
			"BAMBOO_DOOR", "CHERRY_DOOR" };
	private final String[] gates = { "ACACIA_FENCE_GATE", "BIRCH_FENCE_GATE", "DARK_OAK_FENCE_GATE",
			"JUNGLE_FENCE_GATE", "FENCE_GATE", "OAK_FENCE_GATE", "SPRUCE_FENCE_GATE", "CRIMSON_FENCE_GATE",
			"WARPED_FENCE_GATE", "MANGROVE_FENCE_GATE", "BAMBOO_FENCE_GATE", "CHERRY_FENCE_GATE" };
	...
	/**
	 * To deactivate if the player can throw item. "disable-drop-item" config.yml.
	 * 
	 * Desativa que los usuarios que se encuentren en el mundo especificado que no
	 * tengan el permiso sl.staff y que no se encuentren en creativo puedan soltar
	...
	/**
	 * To deactivate if the player can collect item. "disable-pick-up-items"
	 * config.yml.
	 * 
	 * Desativa que los usuarios que se encuentren en el mundo especificado que no
	 * tengan el permiso sl.staff y que no se encuentren en creativo puedan recoger
	 * items.
	 * 
	 * @param e
	 */
	...
	/**
	 * To clean the inventory to enter the server and To clean the chat when
	 * entering the server. "join-clear-inventory" config.yml. "join-clear-chat"
	 * config.yml.
	 * 
	...
	/**
	 * To teleport the player to the spawn when entering the server. "join-spawn"
	 * config.yml.
	 * 
	 ...
	 * @param e
	 *//*
		 * @EventHandler public void onCommandSpawn(PlayerCommandPreprocessEvent e){
		 * if(e.isCancelled()){ return; } Player p = e.getPlayer();
		 * 
		 ...
		 * if(plugin.config.getBoolean("command-spawn")){ e.setCancelled(true);
		 * if(p.hasPermission("superlobby.use")){ plugin.teleportToSpawn(p); }else {
	 ...
	 * 
	 * Permitir entregar los �tem de opciones del inventario en el mundo
	 * especificado al entrar al seridor.
	 * 
	 * @param e
	 */
			...
			if (plugin.isWorldRadius(e.getPlayer().getLocation(), "lobby-items")) {
				for (CoreItem item : plugin.items) {
					if (item.hasPerm(e.getPlayer())) {
						...
						plugin.log.debug("The player " + e.getPlayer().getName()
								+ " do not have permission to receive the item " + item.getName());
					}
				}
			}
					...
					for (CoreItem item : plugin.items) {
						if (item.like(itemInHand, e.getPlayer())) {
							...
							if (item.getVersionCheck() && (CoreViaVersion.getApi() != null)) {
								...
								if (vali) {
									if (item.hasPerm(e.getPlayer())) {
										item.executeCommands(e.getPlayer(), plugin.loader,
												AllString.prefix + AllString.click_wait, AllString.prefix);
										e.getPlayer().updateInventory();
									} else {
										e.getPlayer().sendMessage(
												CoreColor.colorCodes(AllString.prefix + AllString.error_no_permission));
									}
	...
	/**
	 * To prevent players from falling into the void. "void-tp.enable" config.yml
	 * 
			...
			if (plugin.isWorldRadius(e.getPlayer().getLocation(), "void-tp")) {
				if (e.getPlayer().getLocation().getY() < plugin.config.getInt("void-tp.level")) {
					...
					if (k == null || k.isEmpty() || k.size() == 0) {
						...
						plugin.log.debug(
								"Void-TP: The spawn has not been established, trying to send to the spawn by defeto of the world.");
					} else {
						plugin.teleportToSpawn(e.getPlayer());
					}
				}
	...
	/**
	 * To protect the frames. "frame-protect.enable" config.yml
	 * 
	...
	/**
	 * To protect the farm. "farm-protect.enable" config.yml
	 * 
	...
	public void onSoilChangePlayer(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("farm-protect.enable")) {
			if (plugin.isWorldRadius(e.getPlayer().getLocation(), "farm-protect")) {
				...
				if ((e.getAction() == Action.PHYSICAL) && (CoreMaterial.isMaterial(e.getClickedBlock().getType(),
						"SOIL", "SOUL_SOIL", "LEGACY_SOIL"))) {
					e.setCancelled(true);
				}
	...
	/**
	 * To have the launch pad. "jump-pads.enable" config.yml
	 * 
	 ...
	 * "join-boss-bar.enable" config.yml "join-tab.enable" config.yml
	 * "join-titles.enable" config.yml "join-motd.enable" config.yml
	 * "join-action-bar.enable" config.yml
	 * 
	 * Muestra efetos como bossbar,tab,titulos,subtitulos,motd al jugador al entrar
	 * al servidor.
	 * 
	 * @param e
	 */
	...
	public void onJoinEffets(PlayerJoinEvent e) {
		...
		if (plugin.config.getBoolean("join-motd.enable")) {
			List<String> motd = CoreVariables.replaceList(plugin.config.getStringList("join-motd.message"),
					e.getPlayer());
	...
	/**
	 * To deactivate commands. "commands-blacklist.enable" config.yml
	 * 
	...
	/**
	 * To format the chat. "chat-format.enable" chat.yml
	 * 
			...
			if (plugin.permission != null) {
				...
				if (!plugin.permission.getName().equalsIgnoreCase("SuperPerms")) {
					...
					if (plugin.configChat.isSet("chat-format.format." + nonColoredg.toLowerCase())) {
						...
						plugin.log.alert(
								"The " + nonColoredg.toLowerCase() + " group was not found in the configuration.");
						for (String dd : plugin.configChat.getConfigurationSection("chat-format.format")
								.getKeys(false)) {

							if (!nonColoredg.equals(dd)) {
								plugin.log.debug("El grupo '" + nonColoredg.toLowerCase()
										+ "' obtenido del permiso del usuario no es igual al grupo de la configuracion '"
										+ dd + "'");

							} else {
								plugin.log.debug("El grupo '" + nonColoredg.toLowerCase()
										+ "' obtenido del permiso del usuario es igual al grupo de la configuracion '"
										+ dd + "'");
							}
	...
	/**
	 * To execute custom events. "custom-events" config.yml "list" customevents.yml
	 * 
	...
	/**
	 * To activate the menus. "menu" config.yml "list" menus.yml
	 * 
	...
	/**
	 * To activate the Scoreboard. "settings-enable" board.yml
	 * 
	...
	public void onJoinBoard(final PlayerJoinEvent e) {
		if (plugin.configBoard.getBoolean("settings-enable")) {
			if (plugin.configBoard.getStringList("settings-world").contains(e.getPlayer().getWorld().getName())) {
				PlayerBoard board = new PlayerBoard(e.getPlayer(), plugin.configBoard.getStringList("settings-title"),
						plugin.configBoard.getInt("settings-update-title"));
				...
				new BukkitRunnable() {
					public void run() {
						...
						PlayerBoard board = new PlayerBoard(e.getPlayer(),
								plugin.configBoard.getStringList("settings-title"),
								plugin.configBoard.getInt("settings-update-title"));
						board.setupLines(plugin.configBoard.getConfigurationSection("board"));
						board.send();
						plugin.playerBoards.put(e.getPlayer(), board);
				...
				public void run() {
					if (plugin.playerBoards.containsKey(p)) {
						...
						if (plugin.configBoard.getStringList("settings-world").contains(p.getWorld().getName())) {
							PlayerBoard board = new PlayerBoard(p, plugin.configBoard.getStringList("settings-title"),
									plugin.configBoard.getInt("settings-update-title"));
	...
	/**
	 * Check for updates. "update-check" config.yml
	 * 
					...
					public void run() {
						...
						try {
							if (updater.checkForUpdates()) {
								CoreColor.message(p, AllString.prefix
										+ "&7Update avaliable for SuperLobby. Please update to recieve latest version.");
	...
	public void onDispenserClick(PlayerInteractEvent e) {
		...
		if (plugin.config.getBoolean("disable-dispenser-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-dispenser-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (CoreMaterial.isMaterial(e.getClickedBlock().getType(), "DISPENSER")) {
						if ((e.getPlayer().isOp()) || (e.getPlayer().hasPermission("superlobby.staff"))) {
							if (e.getPlayer().getGameMode() == GameMode.CREATIVE) {
								e.setCancelled(false);
							} else {
	...
	public void onNoteBlockClick(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("disable-noteblock-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-noteblock-interaction"))
					&& (!e.isCancelled())) {
	...
	public void onButtonClick(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("disable-button-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-button-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
	...
	public void onTrapdoorClick(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("disable-trapdoor-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-trapdoor-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
	...
	public void onFenceGateClick(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("disable-fence-gate-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-fence-gate-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
	...
	public void onHopperClick(PlayerInteractEvent e) {
		if (plugin.config.getBoolean("disable-hopper-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-hopper-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (CoreMaterial.isMaterial(e.getClickedBlock().getType(), "HOPPER", "HOPPER_MINECART")) {
						if ((e.getPlayer().isOp()) || (e.getPlayer().hasPermission("superlobby.staff"))) {
							if (e.getPlayer().getGameMode() == GameMode.CREATIVE) {
								e.setCancelled(false);
							} else {
	...
	public void onDropperClick(PlayerInteractEvent e) {
		...
		if (plugin.config.getBoolean("disable-dropper-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-dropper-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (CoreMaterial.isMaterial(e.getClickedBlock().getType(), "DROPPER")) {
						if ((e.getPlayer().isOp()) || (e.getPlayer().hasPermission("superlobby.staff"))) {
							if (e.getPlayer().getGameMode() == GameMode.CREATIVE) {
								e.setCancelled(false);
							} else {
	...
	public void onDaylightSensorClick(PlayerInteractEvent e) {
		...
		if (plugin.config.getBoolean("disable-daylight-sensor-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-daylight-sensor-interaction"))
					&& (!e.isCancelled())) {
				if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
					if (CoreMaterial.isMaterial(e.getClickedBlock().getType(), "DAYLIGHT_DETECTOR",
							"DAYLIGHT_DETECTOR_INVERTED", "LEGACY_DAYLIGHT_DETECTOR_INVERTED")) {
						if ((e.getPlayer().isOp()) || (e.getPlayer().hasPermission("superlobby.staff"))) {
							if (e.getPlayer().getGameMode() == GameMode.CREATIVE) {
								e.setCancelled(false);
							} else {
	...
	public void onArmorStandsClick(PlayerArmorStandManipulateEvent e) {
		...
		if (plugin.config.getBoolean("disable-armorstands-interaction.enable")) {
			if ((plugin.isWorldRadius(e.getPlayer().getLocation(), "disable-armorstands-interaction"))
					&& (!e.isCancelled())) {
				if ((e.getPlayer().isOp()) || (e.getPlayer().hasPermission("superlobby.staff"))) {
					if (e.getPlayer().getGameMode() == GameMode.CREATIVE) {
						e.setCancelled(false);
	...
	public void firstJoinDetection(PlayerJoinEvent e) {
		...
		if (!existingPlayer) {
			...
			if (plugin.configFirstJoin.getBoolean("firework.enabled")) {
				for (int i = 0; i < 5; i++) {
					int m = i * 10;
					this.plugin.getInstance().getServer().getScheduler()
							.scheduleSyncDelayedTask(this.plugin.getInstance(), new Runnable() {
								public void run() {
									fireword(player);
								}
							}, m);
			...
			if (plugin.configFirstJoin.getBoolean("written-books.enabled")) {
				CoreWrittenBooks.sendBook(player, plugin.configFirstJoin.getStringList("written-books.pages"),
						plugin.configFirstJoin.getString("written-books.title"),
						plugin.configFirstJoin.getString("written-books.name"),
						plugin.configFirstJoin.getBoolean("written-books.give-item"));
			}
			...
			if (plugin.configFirstJoin.getBoolean("commands.enabled")) {
				for (String comando : plugin.configFirstJoin.getStringList("commands.commands")) {
					CoreExecuteComands c = new CoreExecuteComands(player, CoreVariables.replace(comando, player),
							plugin.getInstance(), AllString.prefix);
					c.execute();
				}
			}
		}
